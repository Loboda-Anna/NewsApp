...
})({"3GsLi":[function(require,module,exports) {
"use strict";
...
const readNews = (0, _addToRead.loadRead)(KEY_ALREADY_READ);
if (readNews) {
    const arrayOfDates = [];
    for (let news1 of readNews)if (!arrayOfDates.includes(news1.dateOfRead)) arrayOfDates.push(news1.dateOfRead);
    ...
    cardLists.forEach((list1)=>{
        for (let news2 of readNews)if (news2.dateOfRead === list1.closest(".read-by-date-container").querySelector(".date-of-read").textContent) {
            const cardMarkup = `
    <li class="news-card__item" >
      <article class="news-card__article">
        <div class="news-card__img-wrapper">
          <img class="news-card__img" src=${news2.urlMedia} alt="news image" />
          <span class="news-card__category">${news2.section}</span>
          <div class="news-card__btn-favorite ${(0, _markup.checkFavoriteStorage)(news2.url) ? "remove-from-fav" : ""}">
            <span>${(0, _markup.checkFavoriteStorage)(news2.url) ? "Remove from favorite" : "Add to favorite"}</span>
            ${(0, _markup.checkFavoriteStorage)(news2.url) ? (0, _markup.activeHeartSvg) : (0, _markup.heartSvg)}
            </div>
        </div>
        <div class="news-card__text">
          <h2 class="news-card__title">${(0, _newsTextHiding.textHiding)(news2.title, 60)}</h2>
          <p class="news-card__description">${(0, _newsTextHiding.textHiding)(news2.abstract, 150)}</p>
        </div>
        <div class="news-card__info">
          <span class="news-card__date">${news2.published_date.replaceAll("-", "/")}</span>
          <a class="news-card__read-more" href=${news2.url} target='blank'>Read more</a>
        </div>
      </article>
    </li>
    `;
            list1.insertAdjacentHTML("beforeend", cardMarkup);
        }
    });
} else (0, _markup.nothingFound)();
...
},{"./add-to-read":"8N0ZO","./header":"bvS82","./markup":"3TIfq","./news-text-hiding":"jrucD"}],"8N0ZO":[function(require,module,exports) {
...
},{"./markup":"3TIfq","date-fns":"9yHCA","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["3GsLi","3frdX"], "3frdX", "parcelRequire7309")

//# sourceMappingURL=read.531d0d2d.js.map
